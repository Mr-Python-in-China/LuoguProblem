...
struct expression_tree{
    ...
    node* make_tree(string::const_iterator& expression){
        ...
        while (*expression!=')'){
            if (*expression=='(') ptr.push(make_tree(++expression));
                ...
                while (!opt.empty()&&opt.top()>=(*expression=='&')){
                    ...
                    tmp->r=ptr.top();ptr.pop();
                    tmp->l=ptr.top();ptr.pop();
                    ptr.push(tmp);
                    opt.pop();
                }
                opt.push(*expression=='&');
...
bool getval(expression_tree::node const* tree){
    if (!tree->l&&!tree->r) return tree->val;
    else if (tree->val){    // and
        bool ans1=getval(tree->l);
        if (!ans1) return ++ansand,false;
        else return getval(tree->r);
    }else{
        bool ans1=getval(tree->l);
        if (ans1) return ++ansor,true;
        else return getval(tree->r);
    }
}
int main(void){
