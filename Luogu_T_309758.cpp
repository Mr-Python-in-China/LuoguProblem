...
int main(void)
{
    ...
    for (list<ins>::iterator it = a.begin(); it != a.end(); it++)
    {
        cin >> it->light;
        l = m, m = r, r = it;
        if (top->light < it->light)
            top = it;
        if (m->light > l->light && m->light > r->light)
            cnt++;
    }
    if (cnt == 0 && top == r)
    {
    ...
    while (leave != 1)
    {
        ...
        if (lt->light < mt->light)
            lt->pos++;
        for (; rt != a.end(); lt++, mt++, rt++)
        {
            if (lt->light > mt->light && lt->light > rt->light)
                mt->pos--;
            if (lt->light < rt->light && mt->light < rt->light)
                mt->pos++;
        }
        if (mt->light < lt->light)
            mt->pos--;
    ...
    for (vector<ui>::const_reference i : ans)
        cout << i << ' ';
    return 0;
}