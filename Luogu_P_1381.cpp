...
int main(void){
    ...
    vector<string> passage(m);
    {
        ...
        for (vector<string>::reference i:passage){
            cin>>i;
            if (mp.count(i)&&!used.count(i)) 
                used.insert(i),word_count++;
        }
    }
    cout<<word_count<<'\n';
    size_t l=0,r=m;
    while (l<r){
        size_t mid=(l+r)>>1;
        size_t maxc=0;
        for (vector<string>::iterator it=passage.begin(),jt=it+mid-1;jt<passage.end();it++,jt++){
            unordered_set<string> used;
            //used.reserve(n);
            size_t find_count=0;
