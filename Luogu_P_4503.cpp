...
template <typename T0, typename T1> struct myhash {
  size_t operator()(pair<T0, T1> const& x) const {
    return T0(x.first) << 32 | T1(x.second);
  }
};
...
template <typename T = li, T P = 1'000'000'007>
constexpr T ksm(ModInt<T, P> a, T n) {
  ModInt<T, P> ans = 1;
  while (n) {
    if (n & 1) ans *= a;
    ans *= ans;
...
int main(void) {
  ...
  size_t n, l, s_s;
  cin >> n >> l >> s_s;
  vector<vector<pair<ModInt<li, P0>, ModInt<li, P1>>>> hs(
      n, vector<pair<ModInt<li, P0>, ModInt<li, P1>>>(l + 1));
  ...
  for (size_t i = 0; i < l; ++i) {
    unordered_map<pair<ModInt<li, P0>, ModInt<li, P1>>, size_t,
                  myhash<ModInt<li, P0>, ModInt<li, P1>>>
        cnt;
