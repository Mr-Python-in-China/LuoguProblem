...
struct SgTree{
    ...
    SgTree *lt,*rt; //左右子树
    SgTree(const size_t& _l,const size_t& _r,const vector<int>& vec):l(_l),r(_r),changed(0),added(0){
        if (r-l==1){
            maxx=vec[l];
            lt=rt=nullptr;
            return;
    ...
    void update(void){
        if (r-l==1) maxx=changed+added;
        else lt->changed=rt->changed=changed,lt->added+=added,rt->added+=added;
        added=changed=0;
    }
    int query(const size_t& _l,const size_t& _r,const int& x){
        update();
