...
int main(void) {
  ...
  ldx.erase(unique(ldx.begin(), ldx.end()), ldx.end()),
      ldy.erase(unique(ldy.begin(), ldy.end()), ldy.end());
  ...
  for (vector<pos>::const_iterator it = p.begin(); it < p.end(); it++)
    for (vector<pos>::const_iterator jt = p.begin(); jt < p.end(); jt++) {
      if (it->x < jt->x &&
          ldx[jt->x] - ldy[it->x] >= abs(int(ldy[it->y] - ldy[jt->y]))) {
        ...
        size_t leftline, rightline;
        {
          ...
          while (l < r) {
            size_t mid = ((r - l) >> 1) + l;
            if (s.get_sum(it->x + 1, 0, jt->x, mid) <
                s.get_sum(it->x + 1, 0, jt->x, miny))
              r = mid - 1;
            else
              l = mid;
          }
