...
int main(void) {
  ...
  for (rect& i : rects) {
    ...
    dstx.push_back(i.ul.x), dstx.push_back(i.dr.x), dsty.push_back(i.ul.y),
        dsty.push_back(i.dr.y);
  }

  // for (const rect& i:rects) cout<<i.ul.x<<','<<i.ul.y<<'
  // '<<i.dr.x<<','<<i.dr.y<<'\n'; cout<<'\n';
  ...
  for (const rect& i : rects)
    ++dif_cnt[i.ul.x][i.ul.y], --dif_cnt[i.ul.x][i.dr.y + 1],
        --dif_cnt[i.dr.x + 1][i.ul.y], ++dif_cnt[i.dr.x + 1][i.dr.y + 1];
  vector<vector<li>> cnt(dstx_size, vector<li>(dsty_size));
  cnt[0][0] = dif_cnt[0][0];
  for (size_t i = 1; i < dstx_size; i++)
    cnt[i][0] = dif_cnt[i][0] + cnt[i - 1][0];
  for (size_t i = 1; i < dsty_size; i++)
    cnt[0][i] = dif_cnt[0][i] + cnt[0][i - 1];
  for (size_t i = 1; i < dstx_size; i++)
    for (size_t j = 1; j < dsty_size; j++)
      cnt[i][j] =
          cnt[i][j - 1] + cnt[i - 1][j] - cnt[i - 1][j - 1] + dif_cnt[i][j];
  uli ans = 0;
  for (size_t i = 1; i < dstx_size; i++)
    for (size_t j = 1; j < dsty_size; j++)
