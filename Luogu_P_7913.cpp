...
struct plane {
  ui arrive, leave;
  bool type;
  struct cmp_arrive_less {
    bool operator()(plane const& a, plane const& b) const {
      return a.arrive < b.arrive;
    }
  };
  struct cmp_leave_greater {
    bool operator()(plane const& a, plane const& b) const {
      return a.leave > b.leave;
    }
  };
  friend bool operator==(plane const& a, plane const& b) {
    return a.arrive == b.arrive;
  }
  struct hash {
    size_t operator()(plane const& a) const { return a.arrive; }
  };
};
template <typename T> T const& debugInfo(T const& val) {
  cerr << val << ' ';
  return val;
}
int main(void) {
  ...
  for (size_t i = 0; i < m1; ++i)
    cin >> a[i].arrive >> a[i].leave, a[i].type = false;
  for (size_t i = m1; i < m1 + m2; ++i)
    cin >> a[i].arrive >> a[i].leave, a[i].type = true;
  ...
  for (plane const& i : a) {
    ...
    if (pool[i.type].empty())
      pool[i.type].push(count.size()), count.push_back(0);
  ...
  vector<size_t> ds[2];
  while (!q[0].empty()) pool[0].push(id[q[0].top()]), q[0].pop();
  while (!q[1].empty()) pool[1].push(id[q[1].top()]), q[1].pop();
  ...
  for (size_t i = 0; i < n - ds[1].size() && i < ds[0].size(); ++i)
    sum += count[ds[0].at(i)];
  ans = max(ans, sum);
  for (size_t i = n - ds[1].size(), j = ds[1].size() - 1;
       i < ds[0].size() && j < ds[1].size(); ++i, --j)
    ans =
        max(debugInfo((sum -= count[ds[1].at(j)]) += count[ds[0].at(i)]), ans);
  cout << ans;
  return 0;
}