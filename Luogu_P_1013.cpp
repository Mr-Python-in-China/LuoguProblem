...
int main(void) {
  ...
  size_t jz = 2;
  for (char i = table[mapper[number1]][mapper[number1]][0]; i != number1;
       i = table[mapper[i]][mapper[number1]][0], ++jz) {
    if (ans[mapper[i]] != -1) ERR();
    ans[mapper[i]] = jz;
  }
  for (ui const& i : ans)
    if (i == -1) ERR();
  for (size_t const& i : ans)
    if (i == -1) ERR();
  for (size_t i = 0; i < n; ++i)
    for (size_t j = 0; j < n; ++j) {
      if ((table[i][j][1] &&
           ans[i] + ans[j] != ans[mapper[table[i][j][0]]] * jz +
                                  ans[mapper[table[i][j][1]]]) ||
