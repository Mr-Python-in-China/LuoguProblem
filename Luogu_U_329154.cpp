...
public:
    STtable(T begin,size_t n,function<uli(T)> const& f):stmax(ceil(log2(n)),vector<T>(n)),gv(f){
        iota(stmax[0].begin(),stmax[0].end(),begin);
        for (uli i=1;i<stmax.size();i++){
            uli len=1<<i;
            for (uli j=0;j+len<=n;j++)
                stmax[i][j]=gv(stmax[i-1][j])>gv(stmax[i-1][j+(len>>1)])?stmax[i-1][j]:stmax[i-1][j+(len>>1)];
        }
    }
    T getmax(size_t l,size_t r){
        --r;
        size_t k=log2(r-l+1);
        return gv(stmax[k][l])>gv(stmax[k][r-(1<<k)+1])?stmax[k][l]:stmax[k][r-(1<<k)+1];
    }
    T getpoint(size_t p){
        return stmax[0][p];
    }
...
int main(void){
    ...
    while (q--){
        ...
        go.add_interval(pl,n+1,maxp-l),go.add_interval(back.get_val(maxp)+1,n+1,r-maxp-1);
        back.add_interval(maxp,n+1,l-maxp),back.add_interval(r,n+1,r-maxp-1);
    #ifdef debug
