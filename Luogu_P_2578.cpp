...
struct board{
    ...
    friend istream& operator>>(istream& in,board& b){
        for (array<unsigned char,3>& i:b.arr) for (unsigned char& j:i)
            in>>j,j-='0';
        return in;
    }
    ...
    board(board const& x,bool op){
        *this=x;
        if (op){
            ...
            arr[2][2]=arr[1][2],arr[1][2]=arr[0][2],
    ...
    board(void){for (size_t i=0;i<3;i++) for (size_t j=0;j<3;j++) arr[i][j]=i*3+j;}
    ...
    inline friend bool operator!=(const board& a,const board& b){return !(a==b);}
    const static board Target;
};
const board board::Target;
namespace std{template<> struct hash<board>{
...
int main(void){
    ...
    queue<board> q({bd});
    while (!q.empty()){
        board p=q.front();q.pop();
        if (p==board::Target){
            vector<board> ans;
            do ans.push_back(p),p=from[p];while (p!=board::Target);
            cout<<ans.size()-1<<'\n';
            for (vector<board>::const_reverse_iterator it=ans.crbegin();it!=ans.crend();it++)
                cout<<*it<<'\n';
            return 0;
        }
        for (char i=0;i<2;i++){
