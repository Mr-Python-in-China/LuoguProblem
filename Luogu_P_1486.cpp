...
template <typename T, typename compare = less<T>> class Unique {
 ...
 public:
  ...
  struct uqCmp {
    bool operator()(Unique const& x, Unique const& y) const {
      return x.val == y.val ? x.id < y.id : x.val < y.val;
    }
  };
  friend istream& operator>>(istream& in, Unique& x) { return in >> x.val; }
  friend ostream& operator<<(ostream& out, Unique const& x) {
    return out << x.val;
  }
...
template <typename T>
using pbds_tree =
    pbds::tree<Unique<T>, pbds::null_type, typename Unique<T>::uqCmp,
  ...
  while (n--) {
    ...
    switch (opt) {
      ...
      case 'S': {
        ...
        cin >> k;
        minmoney += k;
        pbds_tree<int>::iterator end =
            tree.lower_bound(Unique<int>::smallist(minmoney));
#ifdef debug
