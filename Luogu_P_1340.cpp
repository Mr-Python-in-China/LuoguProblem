...
struct dsds{
    ...
    inline size_t find(const size_t& n){return fa[n]==n?n:fa[n]=find(fa[n]);}
    inline void link(const size_t& a,const size_t& b){fa[find(a)]=find(b);}
    inline bool in_one_set(const size_t& a,const size_t& b){return find(a)==find(b);}
};
struct edge{
    size_t u,v;ui w;
    friend inline istream& operator>>(istream& in,edge& e){return in>>e.u>>e.v>>e.w;}
