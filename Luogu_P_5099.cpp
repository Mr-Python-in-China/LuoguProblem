...
struct pos{
    ...
    struct hash{size_t operator()(pos const& p) const{return (size_t(p.x)<<(1<<(sizeof(size_t)>>1)))+p.y;}};
    friend bool operator==(pos const& x,pos const& y){return x.x==y.x&&x.y==y.y;}
...
pos constexpr pos::f[24]={
    {ui(-2),ui(-2)},{ui(-2),ui(-1)},{ui(-2),ui(0)},{ui(-2),ui(+1)},{ui(-2),ui(+2)},
    {ui(-1),ui(-2)},{ui(-1),ui(-1)},{ui(-1),ui(0)},{ui(-1),ui(+1)},{ui(-1),ui(+2)},
    {ui( 0),ui(-2)},{ui( 0),ui(-1)},               {ui( 0),ui(+1)},{ui( 0),ui(+2)},
    {ui(+1),ui(-2)},{ui(+1),ui(-1)},{ui(+1),ui(0)},{ui(+1),ui(+1)},{ui(+1),ui(+2)},
    {ui(+2),ui(-2)},{ui(+2),ui(-1)},{ui(+2),ui(0)},{ui(+2),ui(+1)},{ui(+2),ui(+2)}
};
int main(void){
    ...
    unordered_set<pos,pos::hash> vis({{0,0}});
