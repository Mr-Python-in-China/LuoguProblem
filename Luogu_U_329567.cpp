...
int main(void){
    ...
    for (size_t i=0;i<n;++i){
        ...
        while (!q.empty()){
            ...
            for (size_t const& j:mp[p]) if (dis[i][j]==-1)
    ...
    for (size_t i=1<<abled.size()-1;~i;--i){
        ...
        size_t x=i;
        for (size_t c=0;x;++c,x>>=1) if (x&1) choose.push_back(c);
        if (choose.size()<m) continue;
        size_t maxdis=0;
        for (vector<size_t>::iterator it=choose.begin();it<choose.end();it++) for (vector<size_t>::iterator jt=next(it);jt<choose.end();jt++)
