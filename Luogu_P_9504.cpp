...
int main(void){
    ...
    vector<edge> tmp(m);
    ui maxw=0;
    for (edge& i:tmp) readns::read(i.u),readns::read(i.v),readns::read(i.w),--i.u,--i.v,maxw=max(maxw,i.w);
    graph mp(n*(maxw+2));
    for (edge const& i:tmp){
        size_t const& x=i.u,&y=i.v;ui const& w=i.w;
        for (size_t i=0;i<=maxw;i++)
            mp[x+i*n].push_back({y+i*n+n,w/(i+1)}),mp[y+i*n].push_back({x+i*n+n,w/(i+1)});
        mp[x+(maxw+1)*n].push_back({y+(maxw+1)*n,w/(maxw+1)}),mp[y+(maxw+1)*n].push_back({x+(maxw+1)*n,w/(maxw+1)});
    }
    for (size_t i=0;i<=maxw;i++) mp[s+i*n].push_back({s+i*n+n,0});
#ifdef debug
    for (size_t i=0;i<mp.size();i++){
        cout<<i<<':';
        for (pair<size_t,ui> const& j:mp[i]) cout<<j.first<<','<<j.second<<' ';
        cout<<'\n';
    }
#endif
