...
struct Instruction{
    ...
    struct Output{Value v;};
    struct Move{Value from;Shift to;};
    struct Jump{vector<Instruction>::const_iterator jp;};
    struct JumpIfNot{vector<Instruction>::const_iterator jp;};
    struct Add{Value num;};
    struct Sub{Value sub;};
    union Parm{
        Output out;
        Move mov;
        Jump jmp;
...
namespace compile{
    ...
    Shift stack=0;
    bool is_block_end(string const& s){return regex_match(s.c_str(),regex(R"(\s*\}\s*)"));}
    bool is_varblock(string const& s){return regex_match(s.c_str(),regex(R"(\s*\{\s*vars\s*)"));}
    bool is_yosoro(string const& s){return regex_match(s.c_str(),regex(R"(\s*\:yosoro\s.*)"));}
        ...
        for (;!is_block_end(*beg);beg++){
            ...
            if (vartype=="int"){
                ...
                if (arrtype=="int"){
                    ...
                    stack+=sz;
                }else cerr<<"TypeError: Uknown type "<<vartype<<" of "<<varname,exit(1);
            }else cerr<<"TypeError: Uknown type "<<vartype<<" of "<<varname,exit(1);
        }
        return beg;
    }
