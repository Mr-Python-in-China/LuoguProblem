...
struct people{
    li x,y;uc shifted=0;
    inline friend istream& operator>>(istream& in,people& p){return in>>p.x>>p.y;}
    inline friend ostream& operator<<(ostream& out,const people& p){return out<<p.x+1<<','<<p.y+1<<':'<<ui(p.shifted);}
    ...
    people()=default;
    friend inline bool operator==(const people& a,const people& b){return a.x==b.x&&a.y==b.y&&a.shifted==b.shifted;}
};
namespace std{
    template<> struct hash<people>:public __hash_base<li,people>{
        inline li operator()(people __val) const noexcept{
            return ((hash<li>()(__val.x)<<18)+(hash<li>()(__val.y)<<2)+__val.shifted);
        }
    };
}
int main(void){
    ...
    unordered_map<people,li> vis;vis[people(0,0)]=0;
    queue<people> q({people(0,0)});
    while (!q.empty()){
        people p=q.front();q.pop();
        //cout<<p<<":\n";
        if (p.x==h-1&&p.y==w-1) cout<<vis[p],exit(0);
        for (people::op i=people::op::Left;i<=people::op::Down;i=people::op(uc(i)+1)){
            people np(p,i);
            if (np.x>=0&&np.y>=0&&np.x<h&&np.y<w&&mp[np.x][np.y]&&!vis.count(np)) 
                vis[np]=vis[p]+1,q.push(np)/*,cout<<uc(i)+1<<"->"<<np<<'\n'*/;
        }
        if (p.shifted==0){
            people np(p);np.x+=d,np.y+=r,np.shifted=1;
            if (np.x>=0&&np.y>=0&&np.x<h&&np.y<w&&mp[np.x][np.y]&&!vis.count(np))
