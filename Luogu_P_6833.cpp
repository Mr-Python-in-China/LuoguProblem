...
void dijkstra(vector<vector<size_t>> const& mp, vector<vector<uli>>& dis, pos s,
              size_t const& n, size_t const& m) {
  ...
  while (!q.empty()) {
    ...
    if (!vis[p.x][p.y]) {
      ...
      for (pos const& i : pos::f) {
        pos np = p + i;
        if (np.x < n && np.y < m &&
            dis[np.x][np.y] > dis[p.x][p.y] + mp[np.x][np.y])
...
int main(void) {
  ...
  dijkstra(arr, disa, {0, --a}, n, m), dijkstra(arr, disb, {n - 1, --b}, n, m),
      dijkstra(arr, disc, {n - 1, --c}, n, m);
#ifdef debug
  for (vector<vector<uli>> const& a : {disa, disb, disc})
    for (size_t i = 0; i < n; i++) {
      for (size_t j = 0; j < m; j++) cout << a[i][j] << ' ';
      cout << '\n';
    }
#endif
  uli ans = -1;
