...
struct Direction {
  ...
  Direction operator<<=(unsigned long long int x) {
    x %= 4;
    while (x--) left();
    return *this;
  }
  ...
  Direction operator<<(unsigned long long int x) const {
    return Direction(*this) <<= x;
  }
  Direction operator>>(unsigned long long int x) const {
    return Direction(*this) >>= x;
  }
...
struct Position {
  ...
  bool in_area(const Position UL, const Position DR) {
    return x >= UL.x && x < DR.x && y >= UL.y && y < DR.y;
  }
};
const Position Direction::F[4] = {
    {-1,  0},
    { 0, -1},
    { 1,  0},
    { 0,  1}
...
template <typename T> class Array2D {
 ...
 public:
  ...
  Array2D(size_t _x, size_t _y)
      : arr(vector<vector<T>>(_x, vector<T>(_y))), x(_x), y(_y) {}
  Array2D(size_t _x, size_t _y, T val)
      : arr(vector<vector<T>>(_x, vector<T>(_y, val))), x(_x), y(_y) {}
...
struct actor {
  ...
  actor(Array2D<char>& _mp, const Position _pos, const Direction _f)
      : mp(_mp), pos(_pos), f(_f) {}
  void forward(void) { pos += f.F[f]; }
  void backword(void) { pos -= f.F[f]; }
  void left(void) { f.left(); }
  void right(void) { f.right(); }
  void move(void) {
    (pos + f.F[f]).in_area({0, 0}, {mp.x, mp.y}) && !mp[pos + f.F[f]]
        ? forward()
        : left();
  }
