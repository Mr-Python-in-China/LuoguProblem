...
struct rgb{
    ...
    inline friend bool operator!=(const rgb& a,const rgb& b){return !(a==b);}
};
struct pos:pair<size_t,size_t>{
    using pair<size_t,size_t>::pair;
    size_t& x=first,&y=second;
};
namespace std{template<> struct hash<pos>{
...
int main(void){
    ...
    while (q--){
        size_t i,j;char t;ui c;cin>>i>>j>>t>>c;--i,--j;
        switch (t){
        case '1':mp[i][j].r+=c;break;
        case '2':mp[i][j].g+=c;break;
        case '3':mp[i][j].b+=c;break;
        }
        if (mp[i][j]!=mp[i][m-1-j]) unasf.insert({i,min(j,m-1-j)});
