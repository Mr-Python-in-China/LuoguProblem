...
template<typename T,size_t N> matrix<T,N,N> quick_pow(matrix<T,N,N> a,size_t k){
    matrix<T,N,N> ans;
...
int main(void){
    ...
    size_t k,n;
    cin>>n>>k;
    for (size_t i=0;i<n;++i) for (size_t j=0;j<n;++j) cin>>a.m[i][j];
    a=quick_pow(a,k);
    for (size_t i=0;i<n;++i){for (size_t j=0;j<n;++j) cout<<a.m[i][j]<<' ';cout.put('\n');}
    return 0;
}