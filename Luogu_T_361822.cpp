...
void dfs(graph const& mp, vector<size_t> const& fa, vector<size_t> const& dis,
         vector<bool>& vis, graph& ans, size_t p) {
  vis[p] = true;
  for (size_t const& i : mp[p])
    if (!vis[i])
      if (fa[i] == p)
        dfs(mp, fa, dis, vis, ans, i);
      else if (dis[p] < dis[i]) {
        size_t me = p, it = i;
        while (fa[me] != fa[it])
          if (dis[me] >= dis[it])
...
struct myhash {
  size_t operator()(pair<size_t, size_t> const& x) const noexcept {
...
int main(void) {
  ...
  for (pair<size_t, size_t> const& i : unique_edge)
