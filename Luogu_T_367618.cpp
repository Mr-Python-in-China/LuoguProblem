...
size_t get_childtree_dissum(graph const& mp,vector<size_t>& sz,size_t p,size_t fa=-1){
    sz[p]=1;
    size_t ans=0;
    for (pair<size_t,lli> const& i:mp[p]) if (i.first!=fa){
        (ans+=get_childtree_dissum(mp,sz,i.first,p)+i.second*sz[i.first])%=P;
        sz[p]+=sz[i.first];
    }
    return ans;
}
void get_dissum(graph const& mp,vector<lli>& ans,vector<size_t> const& sz,size_t p,size_t fa=-1){
    for (pair<size_t,lli> const& i:mp[p]) if (i.first!=fa){
        ans[i.first]=((ans[p]-i.second*sz[i.first]+i.second*(sz[0]-sz[i.first]))%P+P)%P;
...
int main(void){
    ...
    vector<size_t> sz(n);vector<lli> dissum(n);
