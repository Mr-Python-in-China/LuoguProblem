...
template <typename T> struct FenwickTree2dDifference {
  FenwickTree2d<T> t;
  FenwickTree2dDifference(size_t n, size_t m): t(n, m) {}
  void add(size_t x1, size_t y1, size_t x2, size_t y2, T const& x) {
    t.add(x1, y1, x), t.add(x2, y2, x), t.add(x1, y2, -x), t.add(x2, y1, -x);
  }
  T get(size_t x, size_t y) { return t.prefix_sum(x + 1, y + 1); }
};
int main(void) {
  ios::sync_with_stdio(false), cin.tie(nullptr), cout.tie(nullptr);
  size_t n, m;
  cin >> n >> m;
  FenwickTree2dDifference<li> s(n, m);
  for (char t; cin >> t;)
    if (t == '1') {
      size_t x1, x2, y1, y2;
      li x;
