...
struct robot {
  ...
  inline friend bool operator==(const robot a, const robot b) {
      ...
      switch (i) {
        ...
        case Walk:
          if (mp[np.x - 1 - F[np.f][0]][np.y - 1 - F[np.f][1]] ||
            ...
            goto No;
        case Creep:
          if (mp[np.x - 1][np.y - 1] || mp[np.x - 1][np.y] ||
              mp[np.x][np.y - 1] || mp[np.x][np.y])
            goto No;
      }
      // cout<<p<<"->"<<np<<'\n',
