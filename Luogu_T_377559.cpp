...
int main(void){
    ...
    for (size_t i=2;i*i<is_prime.size();i++) if (is_prime[i])
    ...
    while (T--){
        ...
        for (vector<ui>& i:arr) for (ui& j:i) cin>>j;
        vector<vector<size_t>> ans(n,vector<size_t>(m));
        for (size_t j=0;j<m;j++) 
            ans[0][j]=is_prime[arr[0][j]];
        for (size_t i=1;i<n;i++) for (size_t j=0;j<n;j++)
            ans[i][j]=is_prime[arr[i][j]]?ans[i-1][j]+1:0;
        for (vector<size_t> const& i:ans){for (size_t const& j:i) cout<<j<<' ';cout<<'\n';}
    }
    return 0;
}