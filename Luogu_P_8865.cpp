...
int main(void){
    ...
    cin>>T>>id;
    while (T--){
        size_t n,m;bool enble_c,enble_f;uli ansc=0,ansf=0;
        cin>>n>>m>>enble_c>>enble_f;
        vector<vector<bool>> mp(n,vector<bool>(m));
        for (vector<bool>& i:mp) for (vector<bool>::reference j:i){
            char c;
            cin>>c;
            j=c=='0';
        }
        ...
        for (size_t j=0;j<m;++j){
            ...
            for (size_t i=1;i<n;++i)
                if (mp[i][j]){
                    (ansc+=(ans_c_dl[i][j]=(sum-(mp[i-1][j]?count_line_rtol[i-1][j]-1:0))*(count_line_rtol[i][j]-1))%=P)%=P;
        ...
        for (size_t j=0;j<m;++j){
            ...
            for (size_t i=n-2;i<n;--i)
                if (mp[i][j]) (ansf+=ans_c_dl[i][j]*len++)%=P;
                else len=0;
        }
