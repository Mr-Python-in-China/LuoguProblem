...
template<typename T> struct position{
    ...
    inline position operator+=(const position p){return x+=p.x,y+=p.y,*this;}
    inline position operator-=(const position p){return x-=p.x,y-=p.y,*this;}
    inline friend position operator+(position x,const position y){return x+=y;}
    inline friend position operator-(position x,const position y){return x-=y;}
    static const position<T> F[4];
};
template<typename T> const position<T> position<T>::F[4]={{T(-1),T(0)},{0,T(-1)},{T(1),T(0)},{T(0),T(1)}};
struct solution{
    ...
    solution(void):zero_pos(2,2){
        ...
        table[2][0]=7,table[2][1]=8,table[2][2]=0;
    }
    solution(string s){
        for (uc i=0;i<3;i++) for (uc j=0;j<3;j++){
            table[i][j]=s[i*3+j]-'0';
...
int main(void){
    ...
    while (!q.empty()){
        ...
        for (size_t t=q.size();t>0;t--){
            solution s=q.front();q.pop();
            if (s==to) cout<<ans,exit(0);
            for (uc i=0;i<4;i++){
                solution ns=s;
                if (ns.move(i)) if (!vis.count(ns)) 
                    vis.insert(ns),q.push(ns);
            }
        }
        ans++;
    }
